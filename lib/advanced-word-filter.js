...
class Filter {
	 ...
	 * @param {string} options.similarityPercent - Similarity Percent
	 */
	constructor(options = {}) {
		Object.assign(this, {
			...
			similarityPercent: options.similarityPercent || 1,
		})
	}
	...
	isProfane(string) {
		...
		return this.list
			.filter((word) => {
				...
				return !this.exclude.includes(word.toLowerCase()) && (XRegExp("^"+newWord+"$", 'igA').test(string) || (this.similarityPercent < 1 && this.similarityPercent <= stringSimilarity.compareTwoStrings(string, newWord)));
			})
			.length > 0 || false;
	}
